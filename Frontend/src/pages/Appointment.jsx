...
const Appointment = () => {
  ...
  useEffect(()=>{getAvailableSlote()} , [docInfo])
      ...
      <div className='flex gap-3 items-center w-full overflow-x-scroll mt-4' >
        ...
        <div className='flex items-center gap-3 w-full overflow-x-scroll mt-4 ' >
          {docSlots.length && docSlots[slotIndex].map((item , index)=>(
            <p onClick={()=>setSlotTime(item.time )}  key={index} className={`text-sm font-light shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? "bg-primary text-white" : 'text-gray-400 border border-gray-300'} `} >
